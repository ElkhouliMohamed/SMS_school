...
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-gray-900">Détails de la Matière</h1>
        <div class="flex space-x-2">
            @hasrole('admin|teacher')
                <a href="{{ route('subjects.edit', $subject) }}" class="flex items-center bg-yellow-500 text-white px-4 py-2 rounded-md hover:bg-yellow-600 transition duration-200">
    ...
    <div class="bg-white shadow-md rounded-lg p-6 mb-8">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <h3 class="text-lg font-semibold text-gray-700 mb-4">Informations de la Matière</h3>
                <div class="border-t pt-3">
                    <div class="flex py-2 border-b">
                        <span class="font-medium text-gray-600 w-1/3">Nom:</span>
    ...
    @if(isset($subject->timetables) && $subject->timetables->count() > 0)
    <div class="bg-white shadow-md rounded-lg p-6 mb-8">
        <h3 class="text-lg font-semibold text-gray-700 mb-4">Emploi du Temps</h3>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach($subject->timetables as $timetable)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                {{ $timetable->room ?? 'Non définie' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <a href="{{ route('timetables.show', $timetable) }}" class="text-blue-600 hover:text-blue-900">
                                    Voir détails
                                </a>
                            </td>
                        </tr>
    ...
    @hasrole('admin|teacher')
        <div class="flex justify-end">
            <form action="{{ route('subjects.destroy', $subject) }}" method="POST" class="inline">
                @csrf
                @method('DELETE')
                <button type="submit" class="flex items-center bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700 transition duration-200" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette matière ?')">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
