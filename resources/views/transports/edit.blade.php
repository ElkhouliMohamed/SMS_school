...
@section('content')
    <h1 class="text-2xl font-bold text-gray-900 mb-4">Modifier le Transport</h1>
    <form action="{{ route('transports.update', $transport) }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')
        <div>
        ...
        <div>
            ...
            <input type="text" name="route" id="route" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" value="{{ old('route', $transport->route) }}" required>
        </div>
        <div>
            <label for="capacity" class="block text-sm font-medium text-gray-700">Capacité</label>
            <input type="number" name="capacity" id="capacity" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" value="{{ old('capacity', $transport->capacity) }}" required>
        </div>
