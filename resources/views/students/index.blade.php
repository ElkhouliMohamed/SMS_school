...
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        ...
        @hasrole('admin')
            <a href="{{ route('students.create') }}" class="flex items-center bg-green-600 text-white px-4 py-2 rounded-md hover:bg-green-700 transition duration-200">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Parents</th>
                    ...
                    @forelse ($students as $student)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                {{ $student->birth_date ? date('d/m/Y', strtotime($student->birth_date)) : 'Non renseignée' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                    ...
                                    <div class="flex flex-wrap gap-1">
                                        ...
                                        @if($student->guardians->count() > 2)
                                            <span class="px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-800">
                                                +{{ $student->guardians->count() - 2 }}
                                            </span>
                                        @endif
                                    </div>
                                @else
                                    <span class="text-gray-500 italic">Aucun parent</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium flex space-x-2">
                                <a href="{{ route('students.show', $student) }}" class="text-green-600 hover:text-green-900 flex items-center">
                                ...
                                @hasrole('admin')
                                    <a href="{{ route('students.edit', $student) }}" class="text-yellow-600 hover:text-yellow-900 flex items-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    <form action="{{ route('students.destroy', $student) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-600 hover:text-red-900 flex items-center" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cet élève ?')">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-10 text-center text-gray-500">
                                ...
                                <p class="mt-1">Commencez par ajouter un nouvel élève</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
