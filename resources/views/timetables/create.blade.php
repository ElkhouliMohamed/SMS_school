...
@section('content')
    <h1 class="text-2xl font-bold mb-4">Create Timetable Entry</h1>
    <form action="{{ route('timetables.store') }}" method="POST" class="max-w-lg">
        @csrf
        <div class="mb-4">
        ...
        <div class="mb-4">
            ...
            <input type="time" name="start_time" id="start_time" class="w-full border rounded p-2" value="{{ old('start_time') }}">
            @error('start_time')
                <p class="text-red-500 text-sm">{{ $message }}</p>
            @enderror
        </div>
        <div class="mb-4">
            ...
            <input type="time" name="end_time" id="end_time" class="w-full border rounded p-2" value="{{ old('end_time') }}">
            @error('end_time')
                <p class="text-red-500 text-sm">{{ $message }}</p>
            @enderror
        </div>
        <div class="mb-4">
            ...
            <input type="text" name="room" id="room" class="w-full border rounded p-2" value="{{ old('room') }}">
            @error('room')
                <p class="text-red-500 text-sm">{{ $message }}</p>
            @enderror
        </div>
        <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded">Create Timetable Entry</button>
    </form>
@endsection